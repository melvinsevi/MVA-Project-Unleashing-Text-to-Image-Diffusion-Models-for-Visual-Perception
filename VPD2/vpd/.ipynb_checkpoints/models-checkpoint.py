...
def register_hier_output(model):
    ...
    def print_shape(module_name, input_tensor):
        print(f"{module_name} - Input Shape: {input_tensor.shape}")
    ...
    def forward(x, timesteps=None, context=None, y=None, **kwargs):
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            module_name = f"Input Block {i} - {module.__class__.__name__}"
            #print_shape(module_name, h)
            #print(context.shape)
            h = module(h, emb, context)
            hs.append(h)

        module_name = f"Middle Block - {self.middle_block.__class__.__name__}"
        #print_shape(module_name, h)
        h = self.middle_block(h, emb, context)
        out_list = []

        for i_out, module in enumerate(self.output_blocks):
            module_name = f"Output Block {i_out} - {module.__class__.__name__}"
            #print_shape(module_name, h)
...
class UNetWrapper(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.use_attn:
            ...
            #print("avg_attn")
            #print(len(avg_attn["down_self"]))
            #print(avg_attn["down_cross"][0].shape)
            #print(avg_attn["down_cross"][1].shape)
            #print(avg_attn["down_cross"][2].shape)
            #print(avg_attn["down_cross"][3].shape)
            ...
            # Assuming attn32 has shape [1, 40, 32, 32]
            # Extract the first ten attention maps
            first_ten_attn32 = attn32[0, :10]
            first_ten_attn64 = attn64[0, :10]

# Assuming you have imported the necessary libraries for plotting

# Plot and save each attention map as a PNG file
            if self.j < 1000:
                for i in range(6):
                    # Plot and save attention maps for attn32
                    fig, axes = plt.subplots(1, 1, figsize=(8, 8))  # Use only one subplot

                    # Overlay the attention map on a blank canvas for attn32
                    axes.imshow(first_ten_attn32[i].detach().cpu().numpy(), cmap='hot')
                    axes.axis('off')

                    # Save the figure in the current working directory
                    save_path32 = f'attention_map32_{i + 1}_of_{self.j}.png'
                    plt.savefig(save_path32, bbox_inches='tight')
                    plt.close()

                    # Plot and save attention maps for attn64
                    if attn64 is not None:
                        #print("6444444444444")

                        fig, axes64 = plt.subplots(1, 1, figsize=(8, 8))  # Use only one subplot

                        # Overlay the attention map on a blank canvas for attn64
                        axes64.imshow(first_ten_attn64[i].detach().cpu().numpy(), cmap='hot')
                        axes64.axis('off')

                        # Save the figure in the current working directory
                        save_path64 = f'attention_map64_{i + 1}_of_{self.j}.png'
                        #plt.savefig(save_path64, bbox_inches='tight')
                        plt.close()
    ...
    def process_attn(self, avg_attn):
        ...
        for k in self.attn_selector:
            for up_attn in avg_attn[k]:
                #print("UPPPP")
                print((up_attn.shape[1]))
                print(up_attn.shape)
                #print("UPPP2")
                size = int(math.sqrt(up_attn.shape[1]))
                #print(self.attn_selector)
                #print(avg_attn)
                #print("OKKKKKK")
                #print(up_attn.shape)
                #print(up_attn)
